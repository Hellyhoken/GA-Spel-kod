...
namespace Fragsurf.Movement {
    ...
    public class SurfCharacter : NetworkBehaviour, ISurfControllable {
		...
		[SerializeField] private LayerMask playerMovementLayer;
		private Animator animator {
			get {
				if (_animator == null) { return _animator = gameObject.FindComponentInChildWithTag<Animator>("Character"); }
				return _animator;
			}
		}
		...
		public Vector3 colliderOffset = new Vector3(0, 1.1f, 0);
		...
		public override void OnStartAuthority()
		{
			...
			InputManager.Controls.Player.Crouch.performed += ctx => _moveData.crouching = true;

			InputManager.Controls.Player.Crouch.canceled += ctx => _moveData.crouching = false;

			InputManager.Controls.Player.Jump.performed += ctx => _moveData.wishJump = true;

			InputManager.Controls.Player.Jump.canceled += ctx => _moveData.wishJump = false;
		}
		...
		private void Animate() {
			animator.transform.position = transform.position - new Vector3(0,0.1f,0);
			animator.transform.rotation = transform.rotation;
			animator.SetBool("Walking", Vector3.Scale(moveData.velocity, new Vector3(1,0,1)).magnitude > 0.05f);
			animator.SetBool("Flying", _groundObject == null);
			animator.SetBool("Crouching", moveData.crouching);
		}
        ...
        private void UpdateMoveData () {
			...
			Vector2 moveInput = InputManager.Controls.Player.Move.ReadValue<Vector2>();
            ...
            bool jump = InputManager.Controls.Player.Jump.ReadValue<float>() != 0;
